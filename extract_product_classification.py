...
def find_product_names(full_text: str, filename: str) -> List[str]:
    ...
    stem = clean_item(re.sub(r'_사업방법서.*$', '', re.sub(r'_사업방법서.*$', '', filename)))
...
def extract_pdf_with_meta(pdf_path: Path) -> Tuple[List[dict], bool]:
    pages_text = []
    ...
    product_names = find_product_names(full_text, pdf_path.name)
    ...
    is_rejoin = '재가입용' in pdf_path.name
    ...
    objects = apply_smart_accident_product_overrides(pdf_path.name, objects)
...
def extract_pdf(pdf_path: Path) -> List[dict]:
    objs, _ = extract_pdf_with_meta(pdf_path)
    return objs


def main() -> None:
    parser = argparse.ArgumentParser(description='Extract 상품분류 JSON from 사업방법서 PDF(s)')
    parser.add_argument('--pdf', type=str, help='single PDF path')
    parser.add_argument('--output', type=str, help='single output JSON path (with --pdf)')
    ...
    for pdf in pdfs:
        objs = extract_pdf(pdf)
        out_name = f"{unicodedata.normalize('NFC', pdf.stem)}.json"
        out = Path(args.output) if args.output and args.pdf else OUTPUT_DIR / out_name
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(json.dumps(objs, ensure_ascii=False, indent=2), encoding='utf-8')
        print(f'{pdf.name} -> {out.name} ({len(objs)} items)')
