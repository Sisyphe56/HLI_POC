...
def extract_pdf_tables(pdf_path: Path) -> List[List[List[str]]]:
    """PDF에서 pdfplumber extract_tables()로 구조화된 테이블 목록을 반환."""
    tables: List[List[List[str]]] = []
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            for table in page.extract_tables() or []:
                # \n 제거 후 normalize_ws
                cleaned = [[normalize_ws((str(cell or '')).replace('\n', '')) for cell in row]
                           for row in table]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='보기개시나이 추출기')
    ...
    parser.add_argument('--output', type=str, help='single output JSON path if --pdf and --json are used')
...
def _load_annuity_blocks(pdf_path: Path) -> List[Dict[str, object]]:
    """테이블 기반 블록(우선) + 텍스트 기반 블록(보완)을 합산하여 반환.
    테이블 기반 추출은 열 헤더로 종신연금형/확정기간연금형을 정확히 구분.
    단, 테이블 블록은 2개 이상의 카테고리 키가 있는 경우에만 사용
    (단일 카테고리는 잘못된 값을 추출할 가능성이 높음).
    텍스트 기반 추출은 항상 포함.
    """
    ...
    # 테이블 블록 중 2개 이상 카테고리 키를 가진 것만 사용 (종신/확정기간 구분이 실제로 있는 경우)
    useful_table_blocks = []
...
def run(pdf_dir: Path, json_dir: Path, output_dir: Path, report_path: Optional[Path]) -> Tuple[int, int]:
    ...
    for pdf_path, json_path in sorted(pairs, key=lambda x: x[1].name):
        lines = extract_pdf_lines(pdf_path)
        annuity_blocks = _load_annuity_blocks(pdf_path)
