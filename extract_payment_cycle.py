...
def extract_payment_cycle_for_pdf(pdf_path: Path, detail_tokens: Optional[Sequence[str]] = None) -> List[dict]:
    product_records = extract_pdf(pdf_path)
    if not product_records:
        return []

    rules, default_cycles = extract_cycle_rules(pdf_path, detail_tokens)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Extract 납입주기 from 사업방법서 PDFs')
    parser.add_argument('--pdf', type=str, help='single PDF path to process')
    parser.add_argument('--output', type=str, help='single output JSON path if --pdf is used')
    parser.add_argument('--max-pages', type=int, default=30, help='limit pages per PDF while parsing (default: 30)')
...
def _main() -> None:
    ...
    for pdf in pdfs:
        records = extract_payment_cycle_for_pdf(pdf, detail_tokens)
        records = _apply_cycle_overrides(records)
        out_name = f"{normalize_ws(pdf.stem)}.json"
        out_path = Path(args.output) if args.output and args.pdf else OUTPUT_DIR / out_name
        out_path.parent.mkdir(parents=True, exist_ok=True)
        out_path.write_text(
            json.dumps(records, ensure_ascii=False, indent=2),
            encoding='utf-8',
        )
        all_records.extend((pdf.name, rec) for rec in records)
        print(f'{pdf.name} -> {out_path.name} ({len(records)} items)')
