...
PDF 사업방법서에서 성별/연령 범위를 추출합니다.

Usage:
    python extract_join_age.py                          # 전체 디렉토리
    python extract_join_age.py --pdf X.pdf --json X.json  # 단일 파일
"""
import argparse
import json
import re
...
def parse_deduction_table(lines: List[str]) -> Tuple[Dict[str, int], Optional[int]]:
    """PDF 텍스트에서 납입기간별 가입최고나이 공제 테이블을 파싱.
...
def parse_variant_age_table(tables: List[List]) -> List[dict]:
    ...
    for table in tables:
        ...
        header0 = [normalize_ws(str(c or '')) for c in table[0]]
        header_lower = [normalize_ws(str(c or '')) for c in table[1]] if len(table) > 1 else []
        gender_row = [normalize_ws(str(c or '')) for c in table[2]] if len(table) > 2 else []

        # 헤더 조건: header_lower에 "구 분"이 있고, gender_row에 "남자"/"여자"가 있고,
        # header0에 종 패턴 (N종) 이 있는 경우
        has_gubun = any('구' in h and '분' in h for h in header_lower)
        has_gender = any('남자' in h or '남 자' in h for h in gender_row)
        has_variant = any(re.search(r'\d+종', h) for h in header0 if h)

        if not (has_gubun and has_gender and has_variant):
            continue
...
def merge_join_age_info(
    ...
    if not age_records and strategy == 'formula' and is_annuity:
        ...
        pdf_paym_deds, pdf_geochi_ded = parse_deduction_table(pdf_lines)
...
def run(
    ...
    for pdf_path, json_path in sorted(pairs, key=lambda x: x[1].name):
        result_rows = process_single(pdf_path, json_path, period_dir)
...
if __name__ == '__main__':
    ...
    if args.dry_run:
        ...
        result_rows = process_single(pdf_path, json_path, args.period_dir)
        ...
        processed_files, processed_rows = run(
            args.pdf_dir, args.json_dir, args.period_dir,
