#!/usr/bin/env python3
"""가입가능보기납기 추출기 (v2 - Word docx).

보험기간, 납입기간, 제2보기개시나이(연금상품) 정보를 Word 문서에서 추출합니다.

Usage:
    python extract_insurance_period_v2.py                              # 전체 디렉토리
    python extract_insurance_period_v2.py --docx X.docx --json X.json  # 단일 파일
"""
import argparse
import json
import re
...
def extract_docx_lines(docx_path: Path) -> List[str]:
    return extract_docx_content(docx_path)[0]
...
def _extract_matrix_invalid_pairs_from_tables(
    tables: List[List[List[str]]],
) -> Set[Tuple[str, str]]:
    """docx 구조화 테이블에서 '-' 표시된 (보험기간, 납입기간) 무효 조합 추출.

    PDF 텍스트 기반 _extract_matrix_invalid_pairs의 docx 테이블 버전.
    테이블 헤더행: ['구분', '20년만기', '60세만기', ...]
    데이터행:      ['20년납', '-', '만19세~38세', ...]
    """
    invalid_pairs: Set[Tuple[str, str]] = set()
    ...
    for table in tables:
        ...
        if not header or '구분' not in header[0]:
            continue
        # 만기 칼럼 추출
        col_periods: List[Tuple[int, str]] = []
        for ci, h in enumerate(header[1:], 1):
            m = re.search(r'(\d+(?:년|세))\s*만기', h)
            if m:
                col_periods.append((ci, m.group(1)))
        ...
        for row in table[1:]:
            ...
            pay_m = re.match(r'^(\d+(?:년|세)\s*납|전기납|일시납)', cell0)
            if not pay_m:
                continue
            pay_label = re.sub(r'\s+', '', pay_m.group(1))
            for ci, ins_label in col_periods:
                if ci < len(row):
...
def merge_period_info(
    ...
    # docx 구조화 테이블에서도 추출 (PDF 텍스트 기반이 빈 결과일 때)
    if not matrix_invalid and pdf_tables:
        matrix_invalid = _extract_matrix_invalid_pairs_from_tables(pdf_tables)
        ...
        for key, cfg in ip_overrides.items():
            ...
            if action == 'fixed':
                ...
                if periods:
                    ...
                    for p in periods:
                        # N-type 납입기간만 제2보기개시나이 복제 (X-type 전기납 제외)
                        if (p.get('최소제2보기개시나이')
...
def run(docx_dir: Path, json_dir: Path, output_dir: Path, report_path: Optional[Path]) -> Tuple[int, int]:
    ...
    for docx_path, json_path in sorted(pairs, key=lambda x: x[1].name):
        result_rows = process_single(docx_path, json_path)
